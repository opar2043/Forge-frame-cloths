...
const CATEGORIES = [
  ...
  { name: "NEW IN" },
  {
    ...
    mega: {
      columns: [
        { title: "Occasion", items: ["Party", "Work", "Casual", "Wedding Guest", "Holiday"] },
        { title: "Length", items: ["Mini", "Midi", "Maxi"] },
        { title: "Season", items: ["Spring", "Summer", "Autumn", "Winter"] },
        { title: "Color", items: ["Black", "White", "Neutrals", "Brights", "Pastels"] },
        { title: "Fabric", items: ["Silk", "Satin", "Linen", "Cotton", "Knit"] },
        { title: "All Dresses", items: ["View All", "New Arrivals", "Best Rated"] },
      ],
      media: {
        title: "Editor’s Pick",
      ...
      <div className="mx-auto  px-4 sm:px-6 lg:px-8 md:mt-5">
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex-1 flex justify-center text-center ">
            <Link to="/" className="text-2xl md:text-4xl font-extrabold tracking-tight text-[#000000] italic md:ml-80">
              Forge & Frame
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center gap-5">
            ...
            <div className="relative">
              ...
              {langOpen && (
                ...
                >
                  <button className="w-full px-3 py-2 text-left hover:bg-gray-50">English</button>
                  <button className="w-full px-3 py-2 text-left hover:bg-gray-50">Nederlands</button>
                </div>
              )}
            </div>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 text-sm text-slate-800"
              >
                United States (USD $) <FiChevronDown size={16} color={ICON_COLOR} />
              </button>
              {regOpen && (
                <div
                ...
                >
                  <button className="w-full px-3 py-2 text-left hover:bg-gray-50">United States (USD $)</button>
                  <button className="w-full px-3 py-2 text-left hover:bg-gray-50">Europe (EUR €)</button>
                  <button className="w-full px-3 py-2 text-left hover:bg-gray-50">United Kingdom (GBP £)</button>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to="/wishlist" aria-label="Wishlist" className="p-2">
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <div
                ...
                onMouseLeave={() => setOpenMega((n) => (n === cat.name ? null : n))}
              >
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 p-6">
                        <div className="lg:col-span-3 grid grid-cols-2 sm:grid-cols-3 gap-4">
                          {cat.mega.columns.map((col) => (
                            <div key={col.title}>
                              <div className="text-[13px] font-bold uppercase text-[#131a25]  tracking-wide text-col mb-2">
                                {col.title}
                              </div>
                              <ul className="space-y-1">
                                {col.items.map((it) => (
                                  <li key={it}>
                                    <Link
                                      to={`${to}${slugify("/" + col.title)}/${slugify(it)}`}
                        ...
                        <div className="lg:col-span-2">
                          <div className="overflow-hidden rounded-xl border border-slate-200">
                            ...
                            <div className="p-4">
                              <div className="font-semibold">{cat.mega.media.title}</div>
                              <p className="text-sm text-slate-600 mt-1">{cat.mega.media.text}</p>
                              <Link to={to} className="mt-3 inline-block text-sm font-semibold underline">
                                Shop now
                              </Link>
                            </div>
                          </div>
          ...
          <div className="absolute left-0 top-0 h-full w-80 max-w-[85%] bg-[#F9F6F2] shadow-xl p-4 overflow-y-auto">
            ...
            <div className="mt-4">
              {CATEGORIES.map((cat) => {
                const to = slugify(cat.name);
                return (
                  <details key={cat.name} className="group border-b border-slate-200">
                    <summary className="flex list-none items-center justify-between py-3 font-semibold text-slate-900">
                      <Link to={to} className={cat.highlight ? "text-rose-400" : ""}>
                        {cat.name}
                      </Link>
                      {cat.mega ? (
            ...
            <div className="py-4 border-t border-slate-200 mt-2">
              <Link to="/wishlist" className="flex items-center gap-2 font-semibold">
            ...
            <div className="py-2 border-t border-slate-200 text-sm">
              <details className="group">
                <summary className="flex list-none items-center justify-between py-2">
                  English <FiChevronDown className="transition group-open:rotate-180" />
                </summary>
              ...
              <details className="group">
                <summary className="flex list-none items-center justify-between py-2">
                  United States (USD $) <FiChevronDown className="transition group-open:rotate-180" />
                </summary>
                <div className="pl-3 pb-2 space-y-2">
                  <button className="block text-left w-full">United States (USD $)</button>
                  <button className="block text-left w-full">Europe (EUR €)</button>
                  <button className="block text-left w-full">United Kingdom (GBP £)</button>
                </div>
              </details>
            </div>
          </div>
